...
//
// Description:    A top module for the MIPS processor that lays out the datapath 
//
//////////////////////////////////////////////////////////////////////////////////
module Datapath(CLK, RST,
    ...
    */
    ...
    wire IFIDStall;     // Pauses the IF/ID reg
    wire PCStall;       // pauses the PC
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    //Instruction Fetch**********************************
    Mux32Bit2To1 IF_Mux(PCAddResult, branchAddress, PCSrc,
                                                                PCSrcOut);
    ProgramCounter IF_ProgramCounter(PCSrcOut, RST, CLK, PCStall, 
                                                                PCResult);
    PCAdder IF_PCAdder(PCResult,
                                PCAddResult);
    InstructionMemory IF_InstructionMemory(PCResult, 
                                                    Instruction);
    
    //IF/ID Pipeline Register****************************
    PipelineRegIF_ID IF_ID(CLK, RST, IFIDStall, PCAddResult, Instruction, 
    ...
    Mux25Bit2To1 ID_CtrlStallMux(Controller, 25'b0, ctrlMuxStall,
                                                                    ControllerMuxOut);
                                                                    
    HazardControl ID_HazCtrlUnit(Instruction_ID[25:21], Instruction_ID[20:16], Controller[24], 
                                 PCSrc_EX, regDstOut, memRead_EX, regWrite_EX, regDstMuxOut_MEM, memRead_MEM,
                                                                                              ctrlMuxStall, IFIDStall, PCStall);                                                                   
                                                                    
    RegisterFile ID_RegisterFile(Instruction_ID[25:21], Instruction_ID[20:16], regDstMuxOut_WB, 
                                     memToRegMuxOut, CLK, regWrite_WB,  
                                                                                        readData1, readData2);   
   
    Mux32Bit2To1 ID_rsFwdMux(readData1, EXEResult_MEM, rsMuxCtrl_ID,
                                                                    rsFwdMuxOut_ID);    
                                                                                                                                                                                                                 
    Mux32Bit2To1 ID_rtFwdMux(readData2, EXEResult_MEM, rtMuxCtrl_ID,
                                                                    rtFwdMuxOut_ID);
    
    BranchModule ID_BranchModule(rsFwdMuxOut_ID, rtFwdMuxOut_ID, Instruction_ID, PCAddResult_ID, Controller[24:20],
                                                                                                         PCSrc, branchAddress, jal);                                                  
                                                            
    Mux32Bit2To1 ID_JalDataMux(rsFwdMuxOut_ID, PCAddResult_ID, jal, 
                                                            readData1_jal);
    
    SignExtension ID_SignExtension(Instruction_ID[15:0], 
                                                        signExtend);
    LowerHalfExtender ID_UpperImmediate(Instruction_ID[15:0], 
                                                            upperImmediate);
    
    Mux5Bit2To1 ID_JalRegMux(Instruction_ID[20:16], 5'b11111, jal,
                                                                    rt_jal);
    ...
    Mux32Bit2To1 EX_ALUSrcA_Mux(rsFwdMuxOut_EX, rtFwdMuxOut_EX, ALUSrcA_EX, 
                                                                    ALUSrcAMuxOut);
    Mux32Bit2To1 EX_ALUSrcB_Mux(rtFwdMuxOut_EX, immediate_EX, ALUSrcB_EX, 
                                                                    ALUSrcBMuxOut);
    
    ALU32Bit EX_ALU(ALUSrcAMuxOut, ALUSrcBMuxOut, HiLoOut, 
                    ALUControl_EX, 
                                    thicc, ALUResult);
    ...
    Mux32Bit2To1 EX_HiLoMux(HiLoOut[31:0], HiLoOut[63:32], hiLoMux_EX, 
                                                                    hiloMuxOut); 
    
    Mux32Bit2To1 EX_ExeDataMux(ALUResult, hiloMuxOut, ALUOutMux_EX, 
                                                                ALUOutMuxOut);
    
    SignExtension EX_RegExtendHalf(rtFwdMuxOut_EX[15:0], 
                                                    regDataSEH);
    ...
    Mux5Bit2To1 EX_RegDestMux(rt_EX, rd_EX, regDst_EX, 
                                                    regDstOut);
    ...
    Mux1Bit2To1 EX_MovMux(regWrite_EX, 1'b1, isMov,     
                                                    movMux);
    ...
    SignExtension MEM_MemExtendHalf(readData[15:0], 
                                                    memDataSEH);
